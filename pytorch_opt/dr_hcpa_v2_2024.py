...
def compute_binary_metrics(y_true: torch.Tensor, y_prob: torch.Tensor, threshold: float = 0.5):
    ...
    specificity = (tn / (tn + fp + 1e-7)).item()  # especificidade no threshold (0.5) — coletada SEMPRE
    specificity_95 = specificity_at_sensitivity(y_true, y_prob)  # spec no ponto de 95% de sensibilidade
