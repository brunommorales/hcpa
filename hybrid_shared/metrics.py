...
def compute_metrics(y_true: torch.Tensor, y_prob: torch.Tensor, threshold: float = 0.5) -> dict:
    ...
    sens = tp / (tp + fn + 1e-7)
    spec = tn / (tn + fp + 1e-7)  # especificidade no threshold (0.5) — coletada SEMPRE
    spec95 = compute_specificity_at_sensitivity(y_true, y_prob)  # spec no ponto de 95% de sensibilidade
    ...
    return {
        ...
        "specificity_at_sens95": float(spec95),
        "f1": float(f1),
        "threshold": threshold,
    }
