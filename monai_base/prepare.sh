...
# Build the optimized MONAI image on a list of nodes.
# Usage: NODELIST=nodeA,nodeB [PARTITION=grace] [TIME_LIMIT=00:20:00] ./prepare.sh

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
IMAGE_X86="monai_base:latest"
IMAGE_ARM="monai_base:latest"
BASE_X86="${BASE_X86:-tf_distributed_x86:latest}"
BASE_ARM="${BASE_ARM:-tf_distributed_arm:latest}"
HOST_UID="$(id -u)"
HOST_GID="$(id -g)"
...
echo "[BUILD] $(hostname) arch=${arch} -> ${image} (base=${base})"
