...
# Build the TensorFlow image on a list of nodes (Grid5000 / Slurm).
# Usage: NODELIST=nodeA,nodeB [PARTITION=poti] [TIME_LIMIT=00:20:00] ./prepare.sh

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
IMAGE_X86="tf_distributed_x86:latest"
IMAGE_ARM="tf_distributed_arm:latest"
BASE_X86="${BASE_X86:-}"
BASE_ARM="${BASE_ARM:-}"
HOST_UID="$(id -u)"
HOST_GID="$(id -g)"
