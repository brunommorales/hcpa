...
# Optional DALI integration (optimized variant)
try:  # pragma: no cover - GPU only
    from nvidia.dali import fn, types as dali_types, tfrecord as dali_tfrec
    from nvidia.dali.pipeline import pipeline_def
    from nvidia.dali.plugin.pytorch import DALIGenericIterator, LastBatchPolicy

    _DALI_AVAILABLE = True
except Exception as _dali_err:  # pragma: no cover - CPU-only environments
    fn = dali_types = dali_tfrec = pipeline_def = DALIGenericIterator = LastBatchPolicy = None
    _DALI_AVAILABLE = False
    _DALI_IMPORT_ERROR = str(_dali_err)
...
class RetinaTFRecordIterableDataset(IterableDataset):
    ...
    def _iter_file(self, tfrec_path: Path) -> Iterator[Dict[str, np.ndarray]]:
        ...
        for record in loader:
            image_bytes = record.get("imagem") or record.get(b"imagem")
            label_raw = record.get("retinopatia") or record.get(b"retinopatia")
...
def create_loaders(
    ...
    *,
    ...
    meta: Dict[str, int] = {
        "train_items": 0,
        "eval_items": 0,
        "train_items_per_rank": 0,
        "eval_items_per_rank": 0,
        "train_steps_per_rank": 0,
        "eval_steps_per_rank": 0,
    }
    ...
    if cfg.use_fake_data or (not train_files and not eval_files):
        ...
        meta.update(
            train_items=cfg.fake_train_size,
            eval_items=cfg.fake_eval_size,
            train_items_per_rank=math.ceil(cfg.fake_train_size / max(world_size, 1)),
            eval_items_per_rank=math.ceil(cfg.fake_eval_size / max(world_size, 1)),
            train_steps_per_rank=math.ceil(cfg.fake_train_size / max(world_size, 1) / max(cfg.batch_size, 1)),
            eval_steps_per_rank=math.ceil(cfg.fake_eval_size / max(world_size, 1) / max(cfg.eval_batch_size, 1)),
        )
        return train_loader, eval_loader, meta

    # Warn if DALI was requested but not available
    if cfg.use_dali and not _DALI_AVAILABLE:
        import warnings
        _err_msg = _DALI_IMPORT_ERROR if '_DALI_IMPORT_ERROR' in dir() else "unknown"
        warnings.warn(
            f"DALI requested (use_dali=True) but not available: {_err_msg}. "
            "Falling back to PyTorch/MONAI data loading. "
            "Install nvidia-dali-cuda120 for GPU-accelerated data loading.",
            RuntimeWarning,
            stacklevel=2,
        )
    ...
    if cfg.use_dali and _DALI_AVAILABLE:
        ...
        total_train_records = sum(count_records(p) for p in train_idx)
        total_eval_records = sum(count_records(p) for p in eval_idx)

        train_steps_per_rank = max(1, math.ceil(total_train_records / max(world_size, 1) / max(cfg.batch_size, 1)))
        eval_steps_per_rank = max(1, math.ceil(total_eval_records / max(world_size, 1) / max(cfg.eval_batch_size, 1)))
        ...
        meta.update(
            train_items=total_train_records,
            eval_items=total_eval_records,
            train_items_per_rank=math.ceil(total_train_records / max(world_size, 1)),
            eval_items_per_rank=math.ceil(total_eval_records / max(world_size, 1)),
            train_steps_per_rank=train_steps_per_rank,
            eval_steps_per_rank=eval_steps_per_rank,
        )

        return DaliWrapper(train_iter, train_steps_per_rank), DaliWrapper(eval_iter, eval_steps_per_rank), meta
    ...
    total_train_records = int(sum(train_counts))
    total_eval_records = int(sum(eval_counts))
    meta.update(
        train_items=total_train_records,
        eval_items=total_eval_records,
        train_items_per_rank=math.ceil(total_train_records / max(world_size, 1)),
