...
def compute_binary_metrics(labels: torch.Tensor, probs: torch.Tensor, threshold: float = 0.5):
    ...
    specificity = (tn / (tn + fp + 1e-8)).item()  # especificidade no threshold (0.5)
    specificity_95 = specificity_at_sensitivity(labels, probs)  # spec @95% sens
    ...
    return {
        ...
        "specificity_at_sens95": specificity_95,
        "f1": f1,
    }
        ...
        if rank == 0:
            ...
            if csv_writer is not None:
                new_row = {
                    ...
                    "val_gpu_util_pct": _val_scope.avg_util_pct,
                    "val_mem_util_pct": _val_scope.avg_mem_util_pct,
                    "val_busy_time_s": _val_scope.busy_time_s,
                    "test_loss": np.nan,
                    "test_auc": np.nan,
    ...
    if rank == 0:
        ...
        if csv_writer is not None:
            final_row = {
                ...
                "test_gpu_mem_avg_mb": (
                    _test_scope.avg_mem_mb if _test_scope.avg_mem_mb is not None else test_mem_avg
                ),
