...
for ax, gpu in zip(axes, gpus):
    ...
    for bar, mean, std in zip(bars, subset["mem_mean"], subset["mem_std"]):
        ax.text(
...
axes[0].set_ylabel("GPU Memory (MB)", fontsize=19)
fig.tight_layout(w_pad=0.5, rect=[0, 0, 1, 0.93])
plt.savefig(
    "/home/users/bmmorales/projects/hcpa/plots_all/memory_faceted_by_gpu.png",
    dpi=300,
    bbox_inches="tight",
)
plt.savefig(
    "/home/users/bmmorales/projects/hcpa/plots_all/memory_faceted_by_gpu.pdf",
    bbox_inches="tight",
)
print("Saved: memory_faceted_by_gpu.png / .pdf")
