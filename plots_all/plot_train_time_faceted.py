...
for ax, gpu in zip(axes, gpus):
    ...
    bars = ax.bar(
        x,
        subset["train_time_mean"],
        yerr=subset["train_time_std"],
        capsize=5,
    ...
    for bar, mean, std in zip(bars, subset["train_time_mean"], subset["train_time_std"]):
        ax.text(
...
axes[0].set_ylabel("Training Time (s)", fontsize=19)
fig.tight_layout(w_pad=0.5, rect=[0, 0, 1, 0.93])
plt.savefig(
    "/home/users/bmmorales/projects/hcpa/plots_all/train_time_faceted_by_gpu.png",
    dpi=300,
    bbox_inches="tight",
)
plt.savefig(
    "/home/users/bmmorales/projects/hcpa/plots_all/train_time_faceted_by_gpu.pdf",
    bbox_inches="tight",
)
print("Saved: train_time_faceted_by_gpu.png / .pdf")
